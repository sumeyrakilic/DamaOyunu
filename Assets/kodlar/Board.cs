...
public class Board : MonoBehaviour
{
  ...
  readonly List<Grid> whiteDirections = new List<Grid>()
    {
        new Grid() {x = 1, y = 1},
        new Grid() {x = -1, y = 1}
    };

  readonly List<Grid> blackDirections = new List<Grid>()
    {
        new Grid() {x = 1, y = -1},
        new Grid() {x = -1, y = -1}
    };
  ...
  public void CalculateMoves(Player currentPlayer)
  {
    ...
    foreach (var item in playerPositions)
    {
      ...
      List<Grid> currentPieceDirection = currentPiece.pieceType == Constants.KING_PIECE ? kingDirections :
                                         currentPiece.player == Player.WHİTE ? whiteDirections
                                         : blackDirections;
      ...
      foreach (Grid tempDirection in currentPieceDirection)
      {
        ...
        if (IsValidGrid(toCheck) && GetPieceAtGrid(toCheck).pieceNumber != -1 && GetPieceAtGrid(toCheck).player != currentPlayer)
        {
  ...
  public GamePiece GetPieceAtGrid(Grid passedPosition)
  {
    ...
    return new GamePiece(Player.WHİTE, -1);
  }
